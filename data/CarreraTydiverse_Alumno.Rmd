...
```{r}
cran%>%filter(country == 'ES')
cran%>%filter(country == 'PT')
...
```{r}
cran2 <- arrange(cran2, desc(ip_id))
cran2
```
12. Ordena `cran2` según los valores de `package e ip_id`.
```{r}
cran2 <- arrange(cran2, package, ip_id)
cran2
```
...
```{r}
cran3 <- cran3%>%mutate( size_gb = size / (2^20 * 2^10))
cran3
```
